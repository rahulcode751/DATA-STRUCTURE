...
int min_cost2(int** input, int m, int n)   /// using Dynamic Programming T(n) = O(n)
{
    int ** dp = new int*[m];
    for (int i = 0; i < m; i++)
    {
...
int min_cost(int** input, int si, int sj, int ei, int ej) /// Using recursion
{
    if (si == ei && sj == ej)
    {
        return input[ei][ej];
    ...
    if (si > ei || sj > ej)
    {
        return INT_MAX;          /// when we are bound of boundary of metrix T(n) = O(3^n)
    }
